...
/// Public value `ClawValue`.
pub const ClawValue = @This();
...
pub fn format(self: ClawValue, allocator: std.mem.Allocator, ctx: FormatContext) FormatError![]u8 {
    ...
    if (self.lay.tag == .list) {
        ...
        if (len > 0) {
            ...
            if (roc_list.bytes) |bytes| {
                var i: usize = 0;
                while (i < len) : (i += 1) {
                    const elem_ptr = bytes + i * elem_size;
                    const elem_val = ClawValue{ .ptr = elem_ptr, .lay = elem_layout };
                    const rendered = try elem_val.format(allocator, ctx);
                    defer allocator.free(rendered);
                    try out.appendSlice(rendered);
    ...
    if (self.lay.tag == .tag_union) {
        unreachable; // tag unions must be formatted via renderValueRocWithType
    }
...
test "equals bool" {
    ...
    var t: [1]u8 = .{1};
    var f: [1]u8 = .{0};
    const vt = ClawValue{ .ptr = &t, .lay = bool_layout, .layout_idx = Idx.bool };
    const vf = ClawValue{ .ptr = &f, .lay = bool_layout, .layout_idx = Idx.bool };
    const ctx = FormatContext{ .layout_store = undefined, .ident_store = null };
